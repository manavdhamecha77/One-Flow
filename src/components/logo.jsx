import { cn } from '@/lib/utils'

export const Logo = ({
    className,
    uniColor
}) => {
    return (
        <div className={cn('flex items-center gap-2 font-bold text-lg', className)}>
            <div className="w-6 h-6 bg-teal rounded-[6px] flex items-center justify-center shadow-none">
                <span className="text-white text-[10px] font-bold tracking-tighter">OF</span>
            </div>
            <span className="font-serif text-xl tracking-tight text-ink">OneFlow</span>
        </div>
    );
}

export const LogoIcon = ({
    className,
    uniColor
}) => {
    return (
        <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={cn('size-5', className)}>
            <path
                d="M3 0H5V18H3V0ZM13 0H15V18H13V0ZM18 3V5H0V3H18ZM0 15V13H18V15H0Z"
                fill={uniColor ? 'currentColor' : 'url(#logo-gradient)'} />
            <defs>
                <linearGradient
                    id="logo-gradient"
                    x1="10"
                    y1="0"
                    x2="10"
                    y2="20"
                    gradientUnits="userSpaceOnUse">
                    <stop stopColor="#9B99FE" />
                    <stop offset="1" stopColor="#2BC8B7" />
                </linearGradient>
            </defs>
        </svg>
    );
}
